...
library(xlsx)
write.xlsx(M, file = 'distances.xlsx', sheetName = "Sheet1", 
           col.names = TRUE, row.names = F, append = FALSE) #just in case something happens with M

#checkpoint
M <- read_excel("distances.xlsx", col_names = T) #read file
head(M)
...
library(readxl)
data <- read_excel("data.xlsx", col_names = T) #read file
head(data)
